...
async def upload_image_and_save_to_db(image: UploadFile = File(...)):
    try:
        ...
        response = await httpx.post(storage_url, files=files)  # storage 서버로 POST 요청 보냄
        response.raise_for_status()  # 오류 발생 시 예외를 일으킴
        ...
        db_response = await httpx.post(db_url, json=image_metadata)  # DB 서버로 POST 요청 보냄
        db_response.raise_for_status()  # 오류 발생 시 예외를 일으킴

        return {"message": "이미지가 성공적으로 업로드되고 메타데이터가 DB에 저장되었습니다."}

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"이미지 업로드 및 메타데이터 저장에 실패했습니다: {str(e)}")
